...
def find_largest_feature(inp_img, scan_tl = None, scan_br=None):
    ...
    #color everything grey (cpmpensates for features outside our middle scanning range)
...
def extract_digit(img, rect, size):
    ...
    digit = cut_from_rect(img, rect) #Get the digit box from whole square
    ...
    if w > 0 and h > 0 and (w * h) > 100 and len(digit) > 0:
        return scal_and_centre(digit, size, 4)
    else:
...
def show_digits(digits, colour=255):
    ...
    rows = []
    with_border = [cv.copyMakeBorder(img.copy(),1,1,1,1,cv2.BORDER_CONSTANT, Npne, colour)for img in digits]
    for i in range(9):
        row = np.concatanate(with_border[i*9:((i+1)*9)], axis=1)
        rows.append(row)
    show_image(np.concatanete(rows))



img = cv2.imread('D:/Code/PySem-FInal/PySem-Final/image1.jpg', cv2.IMREAD_GRAYSCALE)
